...
class BloodBankOrderAdapter(val companies : MutableList<String>) : RecyclerView.Adapter<BloodBankOrderAdapter.BloodBankOrderViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BloodBankOrderViewHolder {
        val v: View
        if(viewType==1){
            v = LayoutInflater.from(parent.context).inflate(R.layout.bloodorderconfirmed,parent,false)
        }
        else if(viewType==2){
            v = LayoutInflater.from(parent.context).inflate(R.layout.bloodorderdenied,parent,false)
        }
        else{
            v = LayoutInflater.from(parent.context).inflate(R.layout.bloodorderpending,parent,false)
        }
