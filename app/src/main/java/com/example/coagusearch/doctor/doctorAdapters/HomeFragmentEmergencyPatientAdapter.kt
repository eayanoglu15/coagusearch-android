...
class HomeFragmentEmergencyPatientAdapter(var emergencyPatientList : MutableList<EmergencyPatientDetail>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HomePagePatientCardViewHolder {
        var v = LayoutInflater.from(parent.context).inflate(R.layout.homepagepatientcard,parent,false)
    ...
    override fun onBindViewHolder(holder: HomePagePatientCardViewHolder, position: Int) {
        ...
        holder.patientName.text=emergencyPatientDetail.userName.toString().capitalize()+" "+emergencyPatientDetail.userSurname.toString().capitalize()
